...
INVITE_AUTHORIZATION_SETTLE_SECONDS = 10
MAX_INVITE_EMAIL_ATTEMPTS = 8
...
async def prepare_replacement_children_for_task(task: Task, account: dict, prefix: str) -> list[str]:
    ...
    for child_id in child_ids:
        if task.status == "stopping":
            ...
            if record:
                await update_child_account(
                    child_id,
                    status="replacement_failed" if record.get("removed_at") else "replacement_stopped",
                    failure_reason=f"补号任务 #{task.id} 已停止",
                )
            continue
        record = find_child_record(records, child_id)
        if not record:
        ...
        try:
            ...
            await update_child_account(
                child_id,
                status="replacement_stopped",
                failure_reason=f"补号任务移除失败：{exc}",
                replacement_reason="旧号未确认移除，可重新补号",
            )
        ...
        for tid in ids:
            if tid in self.tasks:
                ...
                if t.status == "pending":
                    t.status = "stopped"
                    records = await list_child_accounts_raw()
                    for child_id in list(getattr(t, "replacement_child_ids", []) or []):
                        record = find_child_record(records, child_id)
                        if not record:
                            continue
                        await update_child_account(
                            child_id,
                            status="replacement_failed" if record.get("removed_at") else "replacement_stopped",
                            failure_reason=f"补号任务 #{tid} 在开始前已停止",
                            replacement_reason="任务未开始，未执行旧号移除",
                        )
...
async def update_config(item: ConfigUpdate):
    ...
    config = {
        ...
        "self_email_held": config.get("self_email_held", []),
...
async def update_self_emails(item: SelfEmailUpdate):
    ...
    held = normalize_self_email_held(config.get("self_email_held", []))
    config["self_email_accounts"] = item.accounts
    config["self_email_used"] = sorted(email for email in used if email in present)
    config["self_email_held"] = sorted(email for email in held if email in present)
...
async def delete_self_emails(item: SelfEmailDeleteRequest):
    ...
    config["self_email_held"] = sorted(email for email in held if email not in targets)
    config["self_email_cursor"] = len(config["self_email_used"])
...
async def clean_used_self_emails():
    ...
    config["self_email_accounts"] = serialize_self_email_accounts(kept_accounts)
    config["self_email_used"] = []
    config["self_email_held"] = sorted(
        email for email in normalize_self_email_held(config.get("self_email_held", []))
        if email in {account["email"].strip().lower() for account in kept_accounts}
    )
    config["self_email_cursor"] = 0
    self_email_reserved = set()
    save_config(config)
...
async def get_child_accounts(
    ...
    for record in records:
        archived = record.get("status") in ("removed", "replacement_failed", "replaced")
        ...
        if flag in ("low_credits", "exhausted"):
            ...
            record.get("status") == "replacement_failed"
            or (
                record.get("status") == "removed"
                and record.get("replacement_task_id")
    ...
    summary = {
        ...
        "replacing": sum(
            ...
            if item.get("status") in ("replacement_pending", "replacing")
            or (
                item.get("status") == "removed"
                and replacement_task_is_active(item.get("replacement_task_id"))
            )
        ),
        "replacement_failed": sum(
            1 for item in records
            if item.get("status") == "replacement_failed"
            or (
                item.get("status") == "removed"
                and item.get("replacement_task_id")
                and item.get("failure_reason")
                and not replacement_task_is_active(item.get("replacement_task_id"))
            )
        ),
        "replacement_stopped": sum(1 for item in records if item.get("status") == "replacement_stopped"),
        "removed_waiting": sum(
            1 for item in records
            if item.get("status") == "removed" and item.get("replacement_task_id")
        ),
    ...
    for record in records:
        if record.get("status") not in ("removed", "replacement_failed", "replaced") or record.get("external_pool_status") != "imported":
            continue
        pool_id = str(record.get("external_pool_id") or "").strip()
        pool_name = str(record.get("external_pool_name") or "").strip()
        key = (pool_id or pool_name).lower()
    ...
    mother_options = []
    for record in records:
        if record.get("status") in ("removed", "replacement_failed", "replaced"):
            continue
...
async def replace_child_accounts_bulk(req: ChildAccountDeleteRequest):
    ...
    for child_id in ids:
        ...
        if record.get("status") == "replaced":
            ...
            continue
        if record.get("status") in ("replacement_pending", "replacing", "registering", "cleanup_pending"):
            results.append({
                "id": child_id,
                "email": email,
                "ok": False,
                "skipped": True,
                "message": "当前记录正在补号、注册或清理，暂不可重复补号",
            })
            continue
        ...
        async with lock:
            try:
                ...
                for record in group_records:
                    updated = find_child_record(
                        await list_child_accounts_raw(), record.get("id", "")
                    )
                    results.append({
                        "id": record.get("id", ""),
                        "email": record.get("email", ""),
...
def build_authorized_child_record(
    *,
    ...
    return {
        ...
        "workflow_phase": "waiting_sync",
        "login_session_attempts": 0,
        "status": "registering",
...
def build_child_account_record(
    *,
    ...
    return {
        ...
        "login_session_attempts": int(cookie_data.get("login_session_attempts", 0) or 0),
        "external_pool_status": "pending",
        "external_pool_error": "",
        "last_checked_at": "",
        "check_failures": 0,
...
async def create_replacement_batch_task(records: list[dict], reason: str = "自动换号") -> Task:
    ...
    for record in records:
        await update_child_account(
            record.get("id", ""),
            status="removed" if record.get("removed_at") else "replacement_pending",
            replacement_task_id=task.id,
            replaced_at="",
            failure_reason="",
            replacement_reason=(
                f"{reason}，旧号已确认移除，等待补号任务"
                if record.get("removed_at")
                else f"{reason}，等待任务开始后移除旧号"
            ),
        )
    await task_manager.queue.put(task)
    asyncio.create_task(task_manager.start_queue_worker())
...
async def replace_child_account(child_id: str, reason: str = "积分低于阈值") -> dict:
    ...
    async with lock:
        ...
        if record.get("status") in (
            "replacement_pending", "replacing", "registering", "cleanup_pending", "replaced"
        ):
            return record
        task = await create_replacement_task(record, reason)
        updated = find_child_record(await list_child_accounts_raw(), child_id)
        return updated or record


def is_retryable_removed_child(record: dict) -> bool:
    if record.get("status") in ("replacement_stopped", "replacement_failed"):
        return True
    # Backward compatibility for records written by older versions.
    return bool(
        record.get("status") == "removed"
        and record.get("replacement_task_id")
        and record.get("failure_reason")
...
async def repair_stale_replacing_child_accounts() -> int:
    ...
    for record in records:
        ...
        if status not in ("replacement_pending", "replacing") and not legacy_removed:
            continue
...
async def monitor_child_accounts_once(source: str = "manual") -> dict:
    ...
    for record in active:
        email = (record.get("email") or "").strip().lower()
        retryable_replacement = is_retryable_removed_child(record)
        issue_statuses = issues.get(email, {})
        exhausted_pool_ids = sorted(issue_statuses.get("exhausted", set()))
        abnormal_pool_ids = sorted(issue_statuses.get("abnormal", set()))
        issue_sources = [
        ...
        try:
            if issue_sources:
                ...
                updates = {"last_checked_at": now_text()}
                if retryable_replacement:
                    # 已停止/已失败的补号记录不再依赖 Token 池
                    # 本次是否仍返回异常；否则旧任务结束后就会
                    # 从后续监测中永久消失。同时保留旧 Token 来源，
                    # 供补号成功后独立清理。
                    retry_reason = (
                        record.get("replacement_reason")
                    ...
                    matched_accounts.append({
                        ...
                        "pool_ids": [],
                        "pool_names": [],
                        "issue_sources": record.get("token_issue_sources") or [],
                        "issue_labels": [retry_reason],
                    })
                    replacement_records.append(record)
                    replacement_reasons[record.get("id", "")] = retry_reason
                else:
                    updates.update({
                        "exhausted_pool_ids": [],
                        "abnormal_pool_ids": [],
    ...
    for account_id, group_records in grouped_records.items():
        ...
        async with lock:
            try:
                # 监测和手动点击可能几乎同时命中同一母号。
                # 获取母号锁后重读状态，防止用锁外的旧快照
                # 为已进入补号流程的旧号再创建一个重复任务。
                latest_records = await list_child_accounts_raw()
                refreshed_group = []
                for candidate in group_records:
                    latest = find_child_record(latest_records, candidate.get("id", ""))
                    if not latest:
                        continue
                    if latest.get("status") in (
                        "replacement_pending", "replacing", "registering",
                        "cleanup_pending", "replaced",
                    ):
                        continue
                    if replacement_task_is_active(latest.get("replacement_task_id")):
                        continue
                    refreshed_group.append(latest)
                group_records = refreshed_group
                ...
                for record in group_records:
                    current_records = await list_child_accounts_raw()
                    updated = find_child_record(current_records, record.get("id", ""))
                    replacements.append({
                        "child_id": record.get("id", ""),
                        "email": record.get("email", ""),
                        "replacement_task_id": task.id,
...
async def recover_incomplete_child_registrations() -> dict:
    ...
    for record in pending:
        ...
        if result.get("ok"):
            recovered += 1
            if not record.get("registration_started_at"):
                await release_self_email_account(email)
            await update_child_account(
                child_id,
                status="removed",
                workflow_phase="recovered_cleanup",
                removed_at=now_text(),
            ...
            await update_child_account(
                child_id,
                status="cleanup_pending",
                workflow_phase="cleanup_pending",
...
def write_protocol_cookie_result(cookie_id: str, email: str, password: str, record: dict) -> str:
    ...
    data = {
        ...
        "display_name": record.get("display_name", ""),
        "user_id": record.get("user_id", ""),
        "login_session_attempts": int(record.get("login_session_attempts", 0) or 0),
...
def normalize_self_email_held(value) -> set[str]:
    return normalize_self_email_used(value)
...
def build_self_email_overview() -> dict:
    ...
    for idx, account in enumerate(accounts, 1):
        ...
        rows.append({
            ...
            "held": email_key in held,
        })
    used_count = sum(1 for row in rows if row["used"])
    held_count = sum(1 for row in rows if row["held"] and not row["used"])
    return {
        "accounts": rows,
        ...
        "used": used_count,
        "held": held_count,
        "available": len(rows) - used_count - held_count,
    }
...
async def allocate_self_email_account(excluded_emails: set[str] | None = None) -> dict | None:
    global config, self_email_reserved
    excluded = {str(email).strip().lower() for email in (excluded_emails or set())}
    async with self_email_lock:
        accounts = parse_self_email_accounts(config.get("self_email_accounts", ""))
        if not accounts:
            return None
        ...
        for candidate in accounts:
            ...
            if (
                email_key not in used
                and email_key not in held
                and email_key not in excluded
                and email_key not in self_email_reserved
            ):
                account = candidate
...
async def hold_self_email_account(email: str):
    """Persistently keep an authorization-failed mailbox out of allocation.

    It is not counted as permanently used and can be released after cleanup.
    """
    global config, self_email_reserved
    email_key = (email or "").strip().lower()
    if not email_key:
...
async def run_invite_task_legacy(task: Task):
...
async def run_invite_task(task: Task):
    """Run invite-mode tasks as authorize-all -> settle -> register-concurrently."""
    for account_id in task.adobe_account_ids:
        if task.status == "stopping":
            break
        account = find_adobe_account(account_id)
        if not account:
            task.failed += int(task.batch_quantity or 0)
            await task_manager.broadcast(f"[Task#{task.id}] ERROR: selected Adobe admin account no longer exists; skipping")
            continue
        ...
        prefix = f"[Task#{task.id}: {account_label}]"
        target_success = min(
            MAX_INVITE_CHILDREN_PER_ADMIN,
            max(1, int(task.batch_quantity or task.quantity or 1)),
        )
        is_replacement = bool(
            getattr(task, "replacement_child_ids", None)
            or getattr(task, "replacement_child_id", "")
        )
        ...
        if is_replacement:
            ...
            if len(replacement_ids) > MAX_INVITE_CHILDREN_PER_ADMIN:
                ...
                task.replacement_child_id = task.replacement_child_ids[0] if task.replacement_child_ids else ""
                for child_id in overflow_ids:
                    record = find_child_record(await list_child_accounts_raw(), child_id)
                    await update_child_account(
                        child_id,
                        status="replacement_failed" if record and record.get("removed_at") else "replacement_stopped",
                        failure_reason=f"超过单个母号 {MAX_INVITE_CHILDREN_PER_ADMIN} 个子号上限，等待下次补号",
                    )
            prepared_child_ids = await prepare_replacement_children_for_task(task, account, prefix)
            if not prepared_child_ids:
                await task_manager.broadcast(f"{prefix} ⚠️ 没有旧子号被确认移除，补号任务取消")
                continue
            target_success = len(prepared_child_ids)
            task.quantity = target_success
            task.batch_quantity = target_success
        ...
        if task.invite_remove_members:
            ...
            try:
                async with adobe_account_lock:
                    removable = await collect_removable_adobe_member_emails(account)
                    removal_results = await remove_adobe_members(account, removable) if removable else []
                removal_failures = [item for item in removal_results if not item.get("ok")]
                removed_count = len(removal_results) - len(removal_failures)
                await task_manager.broadcast(
                    f"{prefix} 成员清理完成：已移除 {removed_count} 个，失败 {len(removal_failures)} 个"
                )
                if not removal_failures:
                    capacity = {
                        "complete_two_product_count": 0,
        ...
        try:
            ...
            if target_success > remaining_slots:
                await task_manager.broadcast(
                    f"{prefix} 目标超过剩余名额，从 {target_success} 调整为 {remaining_slots}"
                )
                target_success = remaining_slots
        except Exception as exc:
            task.failed += target_success
        ...
        attempt_counts = {slot_index: 0 for slot_index in range(1, target_success + 1)}
        attempted_emails: set[str] = set()
        email_attempts = 0
        account_success = 0

        async def persist_cleanup_pending(
            reserved: dict,
            invite_result: dict,
            reason: str,
        ) -> None:
            reserved["_member_id"] = invite_result.get("member_id", "")
            try:
                pending = await create_authorized_child_account(
                    account=account,
                    reserved=reserved,
                    task=task,
                    product_count=int(invite_result.get("product_count", 0) or 0),
                ...
                await task_manager.broadcast(
                    f"{prefix} ⚠️ [{reserved.get('email', '')}] 待清理记录保存失败：{exc}"
                )

        async def authorize_slot(slot_index: int, initial_reserved: dict | None = None) -> dict | None:
            nonlocal email_attempts
            reserved = initial_reserved
            while attempt_counts[slot_index] < MAX_INVITE_EMAIL_ATTEMPTS:
                if task.status == "stopping":
                    if reserved:
                        await release_self_email_account(reserved.get("email", ""))
                    return None
                if reserved is None:
                    reserved = await allocate_self_email_account(attempted_emails)
                if not reserved:
                    await task_manager.broadcast(
                        f"[Task#{task.id}-{slot_index}: {account_label}] ⚠️ 自备邮箱池已耗尽"
                    )
                    return None

                attempt_counts[slot_index] += 1
                email_attempts += 1
                email = reserved["email"]
                attempted_emails.add(email.strip().lower())
                worker_prefix = f"[Task#{task.id}-{slot_index}: {account_label}]"
                await task_manager.broadcast(
                    f"{worker_prefix} Step 2/3：授权尝试 "
                    f"{attempt_counts[slot_index]}/{MAX_INVITE_EMAIL_ATTEMPTS}，邮箱 {email}"
                )
                try:
                    async with adobe_account_lock:
                        invite_results = await invite_adobe_members(account, [email])
                ...
                product_count = int(invite_result.get("product_count", 0) or 0)
                required_count = max(2, int(invite_result.get("required_product_count", 2) or 2))
                if not invite_result.get("ok") or product_count < required_count:
                    cleanup_result = await cleanup_adobe_member_with_retries(
                        account, email, worker_prefix
                    )
                    task.failed += 1
                    if cleanup_result.get("ok"):
                        await release_self_email_account(email)
                        cleanup_text = "残留成员已清理，邮箱已释放"
                    else:
                        await hold_self_email_account(email)
                        cleanup_text = f"残留成员清理失败：{cleanup_result.get('message', '未知错误')}"
                        await persist_cleanup_pending(
                            reserved,
                            invite_result,
                            f"授权失败且残留成员清理失败：{cleanup_result.get('message', '未知错误')}",
                        )
                    await task_manager.broadcast(
                        f"{worker_prefix} ❌ [{email}] 授权失败："
                        f"{invite_result.get('message', '')}；{cleanup_text}，立即换邮箱补位"
                    )
                    await task_manager.broadcast("__STATE_UPDATE__")
                    reserved = None
                    continue
                ...
                try:
                    ...
                    if cleanup_result.get("ok"):
                        ...
                        await persist_cleanup_pending(
                            reserved,
                            invite_result,
                            f"授权成功但记录保存失败，且残留成员清理失败：{exc}；"
                            f"{cleanup_result.get('message', '未知错误')}",
                        )
                    await task_manager.broadcast(
                        f"{worker_prefix} ❌ [{email}] 授权后无法保存子账号记录：{exc}；"
                        f"成员清理{'成功' if cleanup_result.get('ok') else '失败'}"
                    )
                    reserved = None
                    continue
                ...
                await task_manager.broadcast(
                    f"{worker_prefix} ✅ [{email}] 已确认授权 {product_count}/{required_count} 个产品"
                )
                return {
            ...
            await task_manager.broadcast(
                f"[Task#{task.id}-{slot_index}: {account_label}] ⚠️ 已达到 "
                f"{MAX_INVITE_EMAIL_ATTEMPTS} 个邮箱尝试上限"
            )
            return None

        async def cleanup_authorized_entry(entry: dict, reason: str, release_mailbox: bool) -> None:
            reserved = entry["reserved"]
            pending_child = entry["pending_child"]
            email = reserved["email"]
            worker_prefix = f"[Task#{task.id}-{entry['slot_index']}: {account_label}]"
            cleanup_result = await cleanup_adobe_member_with_retries(account, email, worker_prefix)
            if cleanup_result.get("ok"):
                if release_mailbox:
                    await release_self_email_account(email)
                await update_child_account(
                    pending_child.get("id", ""),
                    status="removed",
                    workflow_phase="stopped" if "停止" in reason else "failed",
                    removed_at=now_text(),
                    cleanup_completed_at=now_text(),
                    failure_reason=reason,
                )
                ...
                await update_child_account(
                    ...
                    status="cleanup_pending",
                    workflow_phase="cleanup_pending",
                    failure_reason=f"{reason}；成员清理失败：{cleanup_result.get('message', '未知错误')}",
                )

        await task_manager.broadcast(
            f"{prefix} Step 2/3：先统一授权 {target_success} 个目标，每个最多换邮箱 "
            f"{MAX_INVITE_EMAIL_ATTEMPTS} 次"
        )
        initial_reservations = await allocate_self_email_batch(target_success)
        authorized_entries: list[dict] = []
        processed_initial_emails: set[str] = set()
        for slot_index in range(1, target_success + 1):
            initial = initial_reservations[slot_index - 1] if slot_index <= len(initial_reservations) else None
            if initial:
                processed_initial_emails.add(initial["email"].strip().lower())
            entry = await authorize_slot(slot_index, initial)
            if entry:
                authorized_entries.append(entry)
            if task.status == "stopping":
                break

        # Release reservations that were preallocated but never consumed.
        for reserved in initial_reservations:
            if reserved["email"].strip().lower() not in processed_initial_emails:
                await release_self_email_account(reserved["email"])

        if task.status == "stopping":
            for entry in authorized_entries:
                await cleanup_authorized_entry(entry, "任务停止，授权成员已清理", True)
            continue
        if not authorized_entries:
            await task_manager.broadcast(f"{prefix} ⚠️ 没有邮箱完成授权，本母号结束")
            continue

        await task_manager.broadcast(
            f"{prefix} ✅ 统一授权完成 {len(authorized_entries)}/{target_success}，"
            f"等待 {INVITE_AUTHORIZATION_SETTLE_SECONDS} 秒同步后开始注册"
        )
        await asyncio.sleep(INVITE_AUTHORIZATION_SETTLE_SECONDS)
        if task.status == "stopping":
            for entry in authorized_entries:
                await cleanup_authorized_entry(entry, "任务停止，授权成员已清理", True)
            continue
        ...
        async def import_and_finalize_success(
            entry: dict,
            cookie_file: str,
            child_record: dict,
            worker_prefix: str,
        ) -> None:
            nonlocal account_success
            replacement_child_id = consume_replacement_child_id(task)
            if replacement_child_id:
                await update_child_account(
                    replacement_child_id,
            ...
            account_success += 1
            await task_manager.broadcast(
                f"{worker_prefix} ✅ [{entry['reserved']['email']}] 注册完成；"
                f"累计成功 {account_success}/{target_success}"
            )
            external_enabled = bool(config.get("token_pool_enabled"))
            try:
        ...
        async def register_slot(initial_entry: dict) -> bool:
            entry = initial_entry
            slot_index = int(entry["slot_index"])
            worker_prefix = f"[Task#{task.id}-{slot_index}: {account_label}]"
            async with sem:
                while entry and task.status != "stopping":
                    reserved = entry["reserved"]
                    pending_child = entry["pending_child"]
                    email = reserved["email"]
                    await mark_self_email_used(email)
                    await update_child_account(
                        pending_child.get("id", ""),
                        status="registering",
                        workflow_phase="registering",
                        registration_started_at=now_text(),
                        login_session_attempts=0,
                        login_session_limit=3,
                        failure_reason="",
                    )
                    try:
                        await task_manager.broadcast(
                        ...
                        cookie_id = (
                            f"task{task.id}_slot{slot_index}_email{attempt_counts[slot_index]}"
                        )
                        cookie_file = await register_invited_self_email(
                            account, reserved, cookie_id, worker_prefix
                        )
                        ...
                        task.failed += 1
                        await task_manager.broadcast(
                            f"{worker_prefix} ❌ [{email}] 三个独立登录会话均失败：{exc}，开始清理成员"
                        )
                        await cleanup_authorized_entry(
                            entry,
                            f"注册失败，已完成会话重试：{exc}",
                            False,
                        )
                        if task.status == "stopping":
                            return False
                        entry = await authorize_slot(slot_index)
                        if not entry:
                            return False
                        if task.status == "stopping":
                            await cleanup_authorized_entry(
                                entry, "任务停止，授权成员已清理", True
                            )
                            return False
                        await task_manager.broadcast(
                            f"{worker_prefix} 新邮箱 {entry['reserved']['email']} 已授权，"
                            f"单独等待 {INVITE_AUTHORIZATION_SETTLE_SECONDS} 秒后注册"
                        )
                        await asyncio.sleep(INVITE_AUTHORIZATION_SETTLE_SECONDS)
                        continue

                    await import_and_finalize_success(
                        entry, cookie_file, child_record, worker_prefix
                    )
                    await task_manager.broadcast("__STATE_UPDATE__")
                    return True
                if entry and task.status == "stopping":
                    await cleanup_authorized_entry(entry, "任务停止，授权成员已清理", True)
                return False

        await task_manager.broadcast(
            f"{prefix} Step 3/3：启动 {len(authorized_entries)} 个注册目标，并发 {max(1, task.concurrency)}"
        )
        workers = [asyncio.create_task(register_slot(entry)) for entry in authorized_entries]
        task.asyncio_tasks = workers
        results = await asyncio.gather(*workers, return_exceptions=True)
        task.asyncio_tasks = []
        account_success = sum(1 for result in results if result is True)
        if account_success >= target_success:
            ...
            await task_manager.broadcast(
                f"{prefix} ⚠️ 未凑满目标，最终成功 {account_success}/{target_success}，"
                f"邮箱尝试 {email_attempts} 次"
            )
    ...
    if replacement_total:
        ...
        for child_id in replacement_pending_ids:
            ...
            await update_child_account(
                ...
                replacement_reason=(
                    "旧号已确认移除，可直接再次补号"
                    if record and record.get("removed_at")
                    else "旧号未确认移除，再次补号时必须先执行移除"
                ),
            )
        if failed_emails:
            await task_manager.broadcast(
